...
function callApi(action, data) {
  return new Promise(function(resolve, reject) {
    ...
    // 2. Standalone mode (Cloudflare Pages / GitHub Pages)
    var apiUrl = CONFIG.getApiUrl();
    if (!apiUrl) {
      console.warn('No API URL configured. Running in local demo mode.');
    ...
    .catch(function(fetchErr) {
      console.warn('Fetch POST failed, attempting JSONP fallback...', fetchErr);
      // Attempt 2: Bulletproof JSONP Fallback (Bypasses CORS completely)
      callJsonp(apiUrl, action, payload)
        .then(resolve)
        .catch(function(jsonpErr) {
          console.error('JSONP fallback also failed:', jsonpErr);
          reject(new Error('ไม่สามารถเชื่อมต่อ Google Apps Script ได้ (กรุณาตรวจสอบว่า Deploy เป็น Anyone และ URL ถูกต้อง)'));
        });
    });
  });
}
